        ...
        try:
            ...
            for i, dict_tm in metcast['metcast'].items():
                str_metcast += f"\t{i}\n\t\tТемпература: {dict_tm['temp']}\n\t\t" \
                               f"Описание:{dict_tm['weather_description']}\n"

            return f"Прогноз погоды по г.{city_name if city_name else self._default_city} " \
                   f" на {metcast['date']}\n{str_metcast}"
...
class OpenWeather(MyWeather):
    ...
    def get_weather(self, city_name=None, date=None):
        ...
        dict_weather = self._parsing_result(html)
